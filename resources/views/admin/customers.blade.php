...
<div x-data="{
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-8">
        <div class="bg-white dark:bg-slate-800 rounded-3xl p-6 border border-slate-100 dark:border-slate-700 shadow-sm dark:shadow-none flex items-center gap-4 group hover:shadow-md dark:hover:border-slate-600 transition-all">
            ...
            <div>
                <p class="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">Total Pelanggan</p>
                <p class="text-2xl font-black text-slate-900 dark:text-white mt-0.5">{{ $totalCustomers }}</p>
            </div>
        </div>
        <div class="bg-white dark:bg-slate-800 rounded-3xl p-6 border border-slate-100 dark:border-slate-700 shadow-sm dark:shadow-none flex items-center gap-4 group hover:shadow-md dark:hover:border-slate-600 transition-all">
            <div class="w-12 h-12 rounded-2xl bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 flex items-center justify-center group-hover:scale-110 transition-transform">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z"/></svg>
            </div>
            <div>
                <p class="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">Pelanggan Baru (Bulan Ini)</p>
                <p class="text-2xl font-black text-slate-900 dark:text-white mt-0.5">{{ $newCustomersThisMonth }}</p>
            </div>
        </div>
        <div class="bg-white dark:bg-slate-800 rounded-3xl p-6 border border-slate-100 dark:border-slate-700 shadow-sm dark:shadow-none flex items-center gap-4 group hover:shadow-md dark:hover:border-slate-600 transition-all">
            <div class="w-12 h-12 rounded-2xl bg-amber-50 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400 flex items-center justify-center group-hover:scale-110 transition-transform">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>
            </div>
            <div>
                <p class="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">Pelanggan Aktif</p>
                <p class="text-2xl font-black text-slate-900 dark:text-white mt-0.5">{{ $customers->where('transactions_count', '>', 0)->count() }}</p>
            </div>
        </div>
        <div class="bg-white dark:bg-slate-800 rounded-3xl p-6 border border-slate-100 dark:border-slate-700 shadow-sm dark:shadow-none flex items-center gap-4 group hover:shadow-md dark:hover:border-slate-600 transition-all">
            ...
            <div>
                <p class="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">Retention Rate</p>
                <p class="text-2xl font-black text-slate-900 dark:text-white mt-0.5">85%</p>
            </div>
        </div>
    </div>
    ...
    <div class="flex flex-wrap items-end justify-between gap-6 mb-6">
        <div class="space-y-1">
            <h2 class="text-xl md:text-2xl lg:text-3xl font-black text-slate-900 dark:text-white tracking-tight">Manajemen Pelanggan</h2>
            <p class="text-sm text-slate-500 dark:text-slate-400 font-medium">Kelola data, pantau loyalitas, dan ekspor riwayat pelanggan.</p>
        </div>
        <div class="flex items-center gap-3">
            <div class="flex items-center bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-1 shadow-sm dark:shadow-none">
                <button class="px-4 py-2 text-xs font-bold text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 rounded-xl transition flex items-center gap-2">
    ...
    {{-- ══════════ Table & Filters ══════════ --}}
    <div class="bg-white dark:bg-slate-800 rounded-[2rem] border border-slate-100 dark:border-slate-700 shadow-sm dark:shadow-none overflow-hidden relative" :class="loading ? 'opacity-60 pointer-events-none' : ''">
        
        {{-- Skeleton Loader (Overlay) --}}
        <div x-show="loading" class="absolute inset-0 z-50 bg-white/40 dark:bg-slate-900/40 flex items-center justify-center backdrop-blur-[1px]">
        ...
        <div class="px-8 py-6 border-b border-slate-50 dark:border-slate-700 bg-slate-50/20 dark:bg-slate-800/80 flex flex-wrap items-center justify-between gap-4">
            <div class="flex items-center gap-3 flex-1 min-w-[300px]">
                <form method="GET" action="{{ route('admin.customers.index') }}" class="flex-1 flex gap-3" @submit="loading = true">
                    <div class="relative flex-1">
                        <svg class="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 dark:text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
                        <input type="text" name="cust_search" placeholder="Cari nama, email, atau telepon..." value="{{ request('cust_search') }}"
                               class="w-full pl-11 pr-4 py-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-600 rounded-2xl text-sm text-slate-800 dark:text-slate-200 placeholder-slate-400 dark:placeholder-slate-500 focus:ring-2 focus:ring-brand outline-none transition-all shadow-sm">
                    </div>
                    <select name="status" onchange="this.form.submit(); loading=true"
                            class="px-4 py-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-600 rounded-2xl text-xs font-bold text-slate-600 dark:text-slate-300 outline-none focus:ring-2 focus:ring-brand shadow-sm">
            ...
            <div x-show="selectedItems.length > 0" x-transition 
                ...
                <span class="text-xs font-bold text-slate-500 dark:text-slate-400"><span x-text="selectedItems.length"></span> dipilih</span>
                <button @click="showBulkDelete = true"
                        class="px-4 py-2.5 bg-red-50 dark:bg-rose-900/30 text-red-600 dark:text-rose-400 text-xs font-bold rounded-xl hover:bg-red-500 hover:text-white dark:hover:bg-rose-600 transition shadow-sm border border-red-100 dark:border-rose-800 flex items-center gap-2">
        ...
        <div class="w-full overflow-x-auto scrollbar-hide rounded-lg">
            <table class="w-full min-w-max">
                <thead>
                    <tr class="bg-slate-50/50 dark:bg-slate-900/50 whitespace-nowrap">
                        <th class="px-8 py-4 text-left">
                            <input type="checkbox" @change="toggleSelectAll" class="w-4 h-4 rounded border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-brand focus:ring-brand cursor-pointer">
                        </th>
                        ...
                        <th class="px-8 py-4 text-right text-[10px] font-bold text-slate-400 dark:text-slate-400 uppercase tracking-widest">Aksi</th>
                    </tr>
                </thead>
                    ...
                    <tr class="group hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors relative" :class="selectedItems.includes('{{ $c->id }}') ? 'bg-blue-50/50 dark:bg-blue-900/20' : ''">
                        ...
                        <td class="px-6 py-5 whitespace-nowrap">
                            ...
                            <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $c->phone) }}" target="_blank" 
                               class="inline-flex items-center gap-1.5 text-xs font-bold text-emerald-600 dark:text-emerald-400 hover:text-emerald-700 dark:hover:text-emerald-300 transition">
                            ...
                            @else
                            <span class="text-xs text-slate-300 dark:text-slate-500 italic">No Phone</span>
                            @endif
                        </td>
                        <td class="px-6 py-5 max-w-xs">
                            <p class="text-xs text-slate-500 dark:text-slate-400 leading-relaxed truncate" title="{{ $c->address }}">
                                {{ $c->address ?? '—' }}
                            </p>
                        </td>
                        ...
                        <td class="px-8 py-5 whitespace-nowrap text-right">
                            <div class="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button @click="openEditCustomer({ id: {{ $c->id }}, name: '{{ addslashes($c->name) }}', email: '{{ addslashes($c->email) }}', phone: '{{ addslashes($c->phone ?? '') }}', address: '{{ addslashes($c->address ?? '') }}' })"
                                        class="w-9 h-9 flex items-center justify-center bg-amber-50 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400 rounded-xl hover:bg-amber-500 hover:text-white dark:hover:bg-amber-600 transition-all shadow-sm dark:shadow-none">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                </button>
                                <button @click="openDeleteCustomer({{ $c->id }}, '{{ addslashes($c->name) }}')"
                                        class="w-9 h-9 flex items-center justify-center bg-red-50 dark:bg-rose-900/30 text-red-500 dark:text-rose-400 rounded-xl hover:bg-red-500 hover:text-white dark:hover:bg-rose-600 transition-all shadow-sm dark:shadow-none">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                                </button>
                            </div>
                        </td>
                        ...
                        <td colspan="6" class="px-8 py-24 text-center">
                            <div class="max-w-xs mx-auto">
                                <div class="w-20 h-20 bg-slate-50 dark:bg-slate-700/50 rounded-full flex items-center justify-center mx-auto mb-6">
                                    <svg class="w-10 h-10 text-slate-300 dark:text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"/></svg>
                                </div>
        ...
        <div class="px-8 py-6 border-t border-slate-50 dark:border-slate-700 bg-slate-50/10 dark:bg-slate-800/80 flex flex-wrap items-center justify-between gap-6">
            <p class="text-xs text-slate-400 dark:text-slate-500 font-bold uppercase tracking-widest">
                Halaman <span class="text-slate-900 dark:text-slate-300">{{ $customers->currentPage() }}</span> dari <span class="text-slate-900 dark:text-slate-300">{{ $customers->lastPage() }}</span>
                <span class="mx-2">•</span> Total <span class="text-slate-900 dark:text-slate-300">{{ $customers->total() }}</span> Pelanggan
            </p>
        ...
        <div class="relative bg-white dark:bg-slate-800 rounded-[2.5rem] shadow-2xl w-full max-w-md z-10 overflow-hidden border dark:border-slate-700">
            ...
            <form method="POST" action="{{ route('admin.customers.store') }}" class="p-8 space-y-5">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div><label class="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2">Telepon</label><input type="text" name="phone" class="w-full px-5 py-3.5 bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-600 text-slate-800 dark:text-slate-200 rounded-2xl outline-none focus:ring-2 focus:ring-brand transition"></div>
    ...
    <div x-show="showEditCustomer" class="fixed inset-0 z-[100] flex items-center justify-center p-4" x-transition>
        <div class="absolute inset-0 bg-slate-900/60 dark:bg-slate-900/80 backdrop-blur-sm" @click="showEditCustomer = false"></div>
        <div class="relative bg-white dark:bg-slate-800 rounded-[2.5rem] shadow-2xl w-full max-w-md z-10 overflow-hidden border dark:border-slate-700">
            <div class="px-8 py-6 bg-amber-500 dark:bg-amber-600 text-white flex justify-between items-center">
            ...
            <form method="POST" :action="`/admin/customers/${editCustomer.id}`" class="p-8 space-y-5">
                @csrf @method('PUT')
                <div><label class="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2">Nama Lengkap</label><input type="text" name="name" required x-model="editCustomer.name" class="w-full px-5 py-3.5 bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-600 text-slate-800 dark:text-slate-200 rounded-2xl outline-none focus:ring-2 focus:ring-amber-500 transition"></div>
                <div><label class="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2">Email</label><input type="email" name="email" required x-model="editCustomer.email" class="w-full px-5 py-3.5 bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-600 text-slate-800 dark:text-slate-200 rounded-2xl outline-none focus:ring-2 focus:ring-amber-500 transition"></div>
                <div><label class="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2">Telepon</label><input type="text" name="phone" x-model="editCustomer.phone" class="w-full px-5 py-3.5 bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-600 text-slate-800 dark:text-slate-200 rounded-2xl outline-none focus:ring-2 focus:ring-amber-500 transition"></div>
                <div><label class="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2">Alamat</label><textarea name="address" rows="2" x-model="editCustomer.address" class="w-full px-5 py-3.5 bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-600 text-slate-800 dark:text-slate-200 rounded-2xl outline-none focus:ring-2 focus:ring-amber-500 transition resize-none"></textarea></div>
                <button type="submit" class="w-full py-4 bg-amber-500 text-white font-black rounded-2xl shadow-xl shadow-amber-100 dark:shadow-none hover:bg-amber-600 transition transform active:scale-95">Simpan Perubahan</button>
            </form>
        </div>
    </div>
    ...
    <div x-show="showDeleteCustomer" class="fixed inset-0 z-[100] flex items-center justify-center p-4" x-transition>
        <div class="absolute inset-0 bg-slate-900/60 dark:bg-slate-900/80 backdrop-blur-sm" @click="showDeleteCustomer = false"></div>
        <div class="relative bg-white dark:bg-slate-800 rounded-[3rem] shadow-2xl w-full max-w-sm z-10 p-10 text-center border dark:border-slate-700">
            <div class="w-20 h-20 bg-red-50 dark:bg-slate-900/50 text-red-500 dark:text-rose-400 rounded-3xl flex items-center justify-center mx-auto mb-6">
            ...
            <h3 class="text-2xl font-black text-slate-900 dark:text-white mb-3">Hapus Pelanggan?</h3>
            <p class="text-sm text-slate-500 dark:text-slate-400 leading-relaxed mb-10">Data <strong x-text="deleteCustomer.name" class="text-slate-900 dark:text-white"></strong> dan seluruh riwayat transaksinya akan dihapus secara permanen.</p>
            <div class="flex gap-4">
                <button @click="showDeleteCustomer = false" class="flex-1 py-4 bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 font-bold rounded-2xl hover:bg-slate-200 dark:hover:bg-slate-600 transition">Batal</button>
                <form method="POST" :action="`/admin/customers/${deleteCustomer.id}`" class="flex-1">
                    @csrf @method('DELETE')
                    <button type="submit" class="w-full py-4 bg-red-500 text-white font-black rounded-2xl shadow-xl shadow-red-100 dark:shadow-none hover:bg-red-600 transition">Ya, Hapus</button>
                </form>
            </div>
        </div>
    </div>
    ...
    <div x-show="showBulkDelete" class="fixed inset-0 z-[100] flex items-center justify-center p-4" x-transition>
        <div class="absolute inset-0 bg-slate-900/60 dark:bg-slate-900/80 backdrop-blur-sm" @click="showBulkDelete = false"></div>
        <div class="relative bg-white dark:bg-slate-800 rounded-[3rem] shadow-2xl w-full max-w-sm z-10 p-10 text-center border dark:border-slate-700">
            <div class="w-20 h-20 bg-red-50 dark:bg-slate-900/50 text-red-500 dark:text-rose-400 rounded-3xl flex items-center justify-center mx-auto mb-6">
                <svg class="w-10 h-10" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
            </div>
            <h3 class="text-2xl font-black text-slate-900 dark:text-white mb-3">Hapus Massal?</h3>
            <p class="text-sm text-slate-500 dark:text-slate-400 leading-relaxed mb-10">Anda akan menghapus <strong x-text="selectedItems.length" class="text-slate-900 dark:text-white"></strong> pelanggan yang dipilih. Tindakan ini tidak dapat dibatalkan.</p>
            <div class="flex gap-4">
                <button @click="showBulkDelete = false" class="flex-1 py-4 bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 font-bold rounded-2xl hover:bg-slate-200 dark:hover:bg-slate-600 transition">Batal</button>
                <form method="POST" action="{{ route('admin.customers.bulkDestroy') }}" class="flex-1">
                    @csrf @method('DELETE')
                    <template x-for="id in selectedItems" :key="id">
                        <input type="hidden" name="ids[]" :value="id">
                    </template>
                    <button type="submit" class="w-full py-4 bg-red-500 text-white font-black rounded-2xl shadow-xl shadow-red-100 dark:shadow-none hover:bg-red-600 transition">Ya, Hapus Semua</button>
                </form>
            </div>
        </div>
    </div>
    ...
    <div x-show="showCustomerTrx" class="fixed inset-0 z-[100] flex items-center justify-center p-4" x-transition>
        <div class="absolute inset-0 bg-slate-900/60 dark:bg-slate-900/80 backdrop-blur-sm" @click="showCustomerTrx = false"></div>
        <div class="relative bg-white dark:bg-slate-800 rounded-[2.5rem] shadow-2xl w-full max-w-2xl z-10 overflow-hidden flex flex-col max-h-[85vh] border dark:border-slate-700">
            <div class="px-8 py-6 bg-slate-900 dark:bg-slate-950 text-white flex justify-between items-center">
                <div>
            ...
            <div class="p-8 overflow-y-auto flex-1">
                ...
                <div x-show="!customerTrxLoading && customerTrx.transactions?.length === 0" class="py-20 text-center">
                     <p class="text-sm font-bold text-slate-400 dark:text-slate-500">Belum ada riwayat transaksi untuk pelanggan ini.</p>
                </div>
                            ...
                            <div class="space-y-2 mb-4 border-b border-slate-200 dark:border-slate-700 pb-4">
                                <template x-for="d in t.details" :key="d.service">
                                    <div class="flex justify-between text-xs">
                                        <span class="text-slate-500 dark:text-slate-400" x-text="`${d.service} (${d.qty} ${d.unit})`"></span>
                                        <span class="font-bold text-slate-900 dark:text-slate-100" x-text="'Rp ' + Number(d.subtotal).toLocaleString('id-ID')"></span>
                                    </div>
                                </template>
                            </div>
                            <div class="flex justify-between items-center">
                                <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Total Bayar</span>
                                <span class="text-lg font-black text-slate-900 dark:text-white" x-text="'Rp ' + Number(t.total_price).toLocaleString('id-ID')"></span>
                            </div>
                        </div>
                    </template>
                </div>
